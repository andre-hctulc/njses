...
import { Instance } from "../main/registery";
...
export class Repository {
    ...
    find<I, D>(service: Instance, id: I): OP<D | null> {
        ...
        return App.registery.invoke<Find<I, D>>(service, FIELD_NAME.REPO_FIND, id);
    }

    findFirst<Q, D>(service: Instance, query: Q): OP<D | null> {
        this.checkRepo(service);
        return App.registery.invoke<FindFirst<Q, D>>(service, FIELD_NAME.REPO_FIND_FIRST, query);
    }

    findMany<Q, D>(service: Instance, query: Q): OP<D[]> {
        this.checkRepo(service);
        return App.registery.invoke<FindMany<Q, D>>(service, FIELD_NAME.REPO_FIND_MANY, query);
    }

    create<I, D>(service: Instance, data: Partial<D>): OP<I> {
        this.checkRepo(service);
        return App.registery.invoke<Create<I, D>>(service, FIELD_NAME.REPO_CREATE, data);
    }

    createMany<I, D>(service: Instance, data: Partial<D>[]): OP<I[]> {
        this.checkRepo(service);
        return App.registery.invoke<CreateMany<I, D>>(service, FIELD_NAME.REPO_CREATE_MANY, data);
    }

    delete<I>(service: Instance, id: I): OP<void> {
        this.checkRepo(service);
        return App.registery.invoke<Delete<I>>(service, FIELD_NAME.REPO_DELETE, id);
    }

    deleteMany<I, Q>(service: Instance, query: Q): OP<I[]> {
        this.checkRepo(service);
        return App.registery.invoke<DeleteMany<I, Q>>(service, FIELD_NAME.REPO_DELETE_MANY, query);
    }

    update<I, D>(service: Instance, id: I, data: Partial<D>): OP<void> {
        this.checkRepo(service);
        return App.registery.invoke<Update<I, D>>(service, FIELD_NAME.REPO_UPDATE, id, data);
    }

    updateMany<I, Q, D>(service: Instance, query: Q, data: Partial<D>): OP<I[]> {
        this.checkRepo(service);
        return App.registery.invoke<UpdateMany<I, Q, D>>(service, FIELD_NAME.REPO_UPDATE_MANY, query, data);
    }
}
